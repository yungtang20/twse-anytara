...
test("mechanical preview renders recommendation and valuation without client calculation", async () => {
  const [view, presentation, evaluator] = await Promise.all([
    read("src/components/views/AIResearchView.tsx"), read("src/lib/aiResearchPresentation.ts"),
    read("server/lib/aiResearchInvestmentConclusion.ts"),
  ]);
